...
public class MusicSelectListener implements KeyListener {
		...
		if (e.getKeyCode() == KeyEvent.VK_ENTER) {
			if (columnIndex == 2) {
				...
				if (selectedSong == "") {
					...
					createSongObj();
					System.out.println(currentSong + "testing");
					JMusicScreenPanel newPanel = new JMusicScreenPanel(currentSong);
					MusicPlayListener newListener = new MusicPlayListener();
					Main.changeScreen(newPanel, newListener);
	...
	/*public void playMusic(){
		...
		try {
		    player.open(new URL("file:\\" + filePath));
		    player.play();
	...
	public void createSongObj(){
		currentSong = new Song(selectedArtist, selectedAlbum, selectedSong, filePath);
	}
